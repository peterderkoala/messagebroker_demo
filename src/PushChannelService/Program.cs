...
using NotificationPlatform.PushChannelService.Consumers;
using NotificationPlatform.PushChannelService.Persistence;
using RabbitMQ.Client;

var builder = WebApplication.CreateBuilder(args);

builder.AddPlatformObservability("push-channel-service");
...
// connection never leaves the compose network. "push_channel" is this
// service's own logical database on the shared Postgres container (ADR-0002).
var connectionString =
    $"Host=postgres;Port=5432;Database=push_channel;Username={postgresUser};Password={postgresPassword}";

builder.Services.AddDbContext<PushChannelDbContext>(options => options.UseNpgsql(connectionString));
...
builder.Services.AddMassTransit(x =>
{
    ...
    x.AddEntityFrameworkOutbox<PushChannelDbContext>(o => o.UsePostgres());
    ...
    x.UsingRabbitMq((context, cfg) =>
    {
        ...
        // automatic push-notification-requested_error queue (issue #2 findings).
        cfg.UseMessageRetry(r => r.Interval(3, TimeSpan.FromSeconds(5)));

        cfg.ReceiveEndpoint("push-notification-requested", e =>
        {
            ...
            e.UseEntityFrameworkOutbox<PushChannelDbContext>(context);
            e.ConfigureConsumer<NotificationRequestedConsumer>(context);

            e.Bind(ChannelRouting.ExchangeName, x =>
            {
                x.ExchangeType = ExchangeType.Topic;
                x.RoutingKey = ChannelRouting.RoutingKeyFor(Channel.Push);
            });
        });
    });
});

builder.Services.AddHealthChecks().AddDbContextCheck<PushChannelDbContext>("postgres");

var app = builder.Build();

app.MapPlatformHealthChecks();

await app.MigrateDatabaseAsync<PushChannelDbContext>();
