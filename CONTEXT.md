...
**Notification**:
The business-level request to tell a user something (e.g. "your order shipped"), independent of how it's delivered. Owned conceptually by the Notification API; realized per channel by a Delivery Attempt.
...
**Event**:
A fact published to the broker that something happened in the domain, e.g. `NotificationRequested`. Named in the past/requested tense and carries only the data a Channel Service needs to act.
...
**Delivery Attempt**:
One Channel Service's persisted record of trying to deliver a Notification: its outcome (Delivered, Failed), timestamp, and the Idempotency Key used to detect duplicates. This is what "notification status" means in this system — there is no separate status concept.
